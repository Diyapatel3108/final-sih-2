...
} from '\var\task\sih-final-main/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer } from '\var\task\sih-final-main/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\sih-final-main')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return tracer.withActiveSpan('Next.js Server Handler', async (span) => {
      span.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\sih-final-main',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\sih-final-main\.netlify\dist\run\handlers\server.js')
